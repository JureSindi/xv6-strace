...
How to run:
bash# Enable priority scheduler
...
Priority inheritance: the low-priority lock holder receives elevated priority when a higher-priority process is waiting
...
Modes:
CommandBehaviorstrace onEnable global tracing for all processesstrace offDisable global tracingstrace run <cmd>Trace a single command onlystrace dumpPrint circular buffer of recent syscall historystrace run -o <file> <cmd>Redirect trace output to a file.
