...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if(ip->type == T_DEV){
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
      return -1;
    return devsw[ip->major].write(ip, src, n);      //comment this (for child)

/*     int stracing = (proc->straced & T_STRACE);       //uncomment from here
    if(stracing) {
      return 1;
    }
    else {
      return devsw[ip->major].write(ip, src, n);        //to here
    } */
  }
